...
namespace Forms9Patch.UWP
{
    public class ImageView : Windows.UI.Xaml.Controls.Grid, IDisposable
    {
        ...
        Windows.UI.Xaml.Controls.Canvas _outlineCanvas;
        //Windows.UI.Xaml.Controls.Canvas _dropShadowCanvas;
        ...
        internal void GenerateOutlineLayout(Windows.Foundation.Size size = default(Windows.Foundation.Size))
        {
            ...
            if (_outlineCanvas != null && Children.Contains(_outlineCanvas))
                Children.Remove(_outlineCanvas);
            //if (_dropShadowCanvas != null && Children.Contains(_dropShadowCanvas))
            //    Children.Remove(_dropShadowCanvas);
            ...
            if (width <= 0 || height <= 0)
                return;
            var rect = new Rect(0, 0, width, height);
            _outlineCanvas = new Windows.UI.Xaml.Controls.Canvas();
            ...
            double separatorWidth = materialButton == null || segmentType == ButtonShape.Rectangle ? 0 : materialButton.SeparatorWidth < 0 ? _roundedBoxElement.OutlineWidth : Math.Max(0, materialButton.SeparatorWidth);
            ...
            var shadowX = Forms9Patch.Settings.ShadowOffset.X  * Display.Scale;
            var shadowY = Forms9Patch.Settings.ShadowOffset.Y * Display.Scale;
            var shadowR = Forms9Patch.Settings.ShadowRadius * Display.Scale;
            var shadowColor = Color.FromRgba(0.0, 0.0, 0.0, 1.0).ToWindowsColor();
            var shadowPadding = RoundedBoxBase.ShadowPadding(RoundedBoxElement as Layout);

            Rect perimeter = rect;
            _dropShadow.Color = Color.Transparent.ToWindowsColor() ;
            ...
            if (makeRoomForShadow)
            {
                ...
                if (!RoundedBoxElement.ShadowInverted)
                {
                    if (segmentType != ButtonShape.Rectangle)
                    {
                        ...
                        Rect result;
                        if (segmentType == ButtonShape.SegmentStart)
                        ...
                        else if (segmentType == ButtonShape.SegmentMid)
                        ...
                        _outlineCanvas.Clip = new Windows.UI.Xaml.Media.RectangleGeometry
                        {
                            Rect = result
                        };
                    }
                    //_dropShadowCanvas = new Windows.UI.Xaml.Controls.Canvas();
                    //Children.Add(_dropShadowCanvas);

                    _dropShadow.BlurRadius = (float)shadowR;
                    _dropShadow.Offset = new System.Numerics.Vector3((float)shadowX, (float)shadowY, 0);
                    _dropShadow.Color = shadowColor;
                    _shadowVisual.Size = new System.Numerics.Vector2((float)width, (float)height);
                }
            }
            ...
            if (drawFill)
            {
                ...
                _outlineCanvas.Children.Add(path);

                if (makeRoomForShadow && !RoundedBoxElement.ShadowInverted)
                    _dropShadow.Mask = path.GetAlphaMask();
            }
            ...
            if (materialButton!=null && RoundedBoxElement.OutlineWidth < 0.05 && separatorWidth > 0 && !_roundedBoxElement.IsElliptical)
            {
                ...
                if (segmentType == ButtonShape.SegmentStart || segmentType == ButtonShape.SegmentMid)
                {
                    if (hz)
                    {
                ...
                if (segmentType == ButtonShape.SegmentMid || segmentType == ButtonShape.SegmentEnd)
                {
                    if (hz)
                    {
                ...
                _outlineCanvas.Children.Add(line);
            ...
            Children.Add(_outlineCanvas);

            //CompositionBrush mask = null;


            /*
            if (_dropShadowCanvas != null)
            {
                var contentVisual = ElementCompositionPreview.GetElementVisual(_outlineCanvas);
                ...
                shadow.BlurRadius = (float)shadowR;
                shadow.Offset = new System.Numerics.Vector3((float)shadowX, (float)shadowY, 0);
                shadow.Color = shadowColor;

                ElementCompositionPreview.SetElementChildVisual(_dropShadowCanvas, sprite);
            }
            */
        ...
        static Rect RoundRect(Rect rect, StackOrientation orientation, Forms9Patch.ButtonShape type)
        {
            return rect;
            /*
        ...
        static Rect SegmentAllowanceRect(Rect rect, double allowance, StackOrientation orientation, Forms9Patch.ButtonShape type)
        {
            Rect result;
            switch (type)
            {
